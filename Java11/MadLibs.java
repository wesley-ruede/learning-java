public class MadLibs {
  /*
  Author: Wesley Ruede
  Date: 9/4/2019
  */
  	public static void main(String[] args){
      String name1 = "Wes";
      String adjective1 = "scruffy";
      String adjective2 = "eager";
      String adjective3 = "colossal";
      String noun1 = "Statue of Liberty";
      String noun2 = "learning";
      String noun3 = "food";
      String noun4 = "fox";
      String noun5 = "gaming";
      String noun6 = "reading";
      String name2 = "Laura";
      int number = 86;
      String place1 = "Netherlands";
      String verb1 = "broadcast";
      
      //The template for the story
     String story = "This morning " +name1+ " woke up feeling " +adjective1+ ". 'It is going to be a " 
     +adjective2+ " day!' Outside, a bunch of " +noun1+ "s were protesting to keep " +noun2+ " in stores. They began to " 
     +verb1+ " to the rhythm of the " +noun3+ ", which made all the " +noun4+ "s very " +adjective3+ ". Concerned, " 
     +name1+ " texted " +name2+ ", who flew " +name1+ " to " +place1+ " and dropped " +name1+ " in a puddle of frozen " 
     +noun5+". "+name1+" woke up in the year "+number+", in a world where "+noun6+"s ruled the world.";

     System.out.println(story);
    }       
}

